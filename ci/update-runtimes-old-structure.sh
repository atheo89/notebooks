...
process_runtime_images() {
    find . -name runtime-images -type d -exec find {} -type f -print \; | grep python-3.11 | while read -r path; do
        ...
        echo "DEBUG the img '${img}'."

        name=$(echo "$path" | sed 's#.*runtime-images/\(.*\)-py.*#\1#')
        echo "DEBUG the name '${name}' "

        py_version=$(echo "$path" | grep -o 'python-[0-9]\.[0-9]*')
        echo "DEBUG the py_version '${py_version}'."

        registry=$(echo "$img" | cut -d '@' -f1)
        echo "DEBUG the registry '${registry}' ."
        ...
        if [[ $name == tensorflow* || $name == pytorch* ]]; then
            ...
            name="minimal-$name"
        elif [[ $name == rocm-pytorch* ]]; then #rocm-pytorch-ubi9 #rocm-runtime-tensorflow-ubi9-python-3.11
            name="${name/rocm-pytorch/rocm-runtime-pytorch}"
        elif [[ $name == rocm-tensorflow* ]]; then #rocm-pytorch-ubi9 #rocm-runtime-tensorflow-ubi9-python-3.11
            name="${name/rocm-tensorflow/rocm-runtime-tensorflow}"
        fi
        if [[ $name == rocm-* ]]; then
            regex="^$name-$py_version-\d+-$HASH\$"
            echo "DEBUG the regex '${regex}' ."
        else
            regex="^runtime-$name-$py_version-\d+-$HASH\$"
            echo "DEBUG the regex '${regex}' ."
        fi

        latest_tag=$(skopeo inspect --retry-times 3 "docker://$img" | jq -r --arg regex "$regex" '.RepoTags | map(select(. | test($regex))) | .[0]')
        echo "DEBUG the latest_tag '${latest_tag}' "
