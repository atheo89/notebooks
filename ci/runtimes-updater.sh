...
for file in $files; do
  ...
  if [[ $TAG_VERSION == main ]]; then
    # This should match with the runtime-image TAG_VERSION name as is on quay.io registry
  ...
  latest_TAG_VERSION=$(skopeo inspect --retry-times 3 "docker://$img" | jq -r --arg regex "$regex" '.RepoTAG_VERSIONs | map(select(. | test($regex))) | .[0]')
  echo "CHECKING: ${latest_TAG_VERSION}"

  if [[ -z "$latest_TAG_VERSION" || "$latest_TAG_VERSION" == "null" ]]; then
    echo "No matching TAG_VERSION found on registry for $file. Skipping."
    continue
  fi

  # Extract the digest sha from the latest TAG_VERSION
  digest=$(skopeo inspect --retry-times 3 "docker://$registry:$latest_TAG_VERSION" | jq .Digest | tr -d '"')
