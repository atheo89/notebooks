...
def discover_config(base_dir: Path) -> tuple[list[str], list[Workbench], list[str], list[Runtime]]:
    ...
    *runtime_resource_files* is the runtime resource list (both in the order
    ...
    # 3) Parse the existing replacement blocks to preserve ordering for current
    # keys, then fill in newly introduced rollout keys from the imagestreams.
    ...
    known_param_keys = {full_key for full_key, _imagestream in params_pairs}

    yaml_param_pairs = [
        (full_key, istream)
        for full_key, istream in _discover_param_pairs_from_imagestreams(base_dir, all_resources)
        if full_key in param_keys and full_key not in known_param_keys
    ]
    params_pairs.extend(yaml_param_pairs)

    discovered_param_keys = {full_key for full_key, _imagestream in params_pairs}
